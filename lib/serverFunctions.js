...
// function to refresh the token in the database
export const refreshToken = async ({ phone, _id }) => {
  try {
    await connectDB()
...
export const setCookies = ({ phone, hour = 4, min = 60 }) => {
  // generating token
  const token = randomBytes(20).toString('hex')
  // setup expire date
  const expireDate = new Date(Date.now() + hour * min * 60 * 1000)
  // set up cookies
